-- PHASE 8D: FIX MISSED SHIFT BUG
-- Melanjutkan dari Phase 8C (Overlapping Shifts).
-- Menambahkan validasi "lewat batas waktu shift" pada preview_attendance_action dan submit_attendance.
...
begin
  ...
  for sched in (
    ...
    if not v_in_exists then
      -- FIX BARU: Kalau shift sudah lewat selesainya dan belum IN, abaikan (skip)
  ...
  -- Jika tidak ada action yang valid dan ada yang missed
  if v_any_missed then
    return json_build_object('success', false, 'error', 'SHIFT_MISSED', 'message', 'Anda melewatkan jam absen untuk shift yang dijadwalkan hari ini');
  end if;

  -- Jika semua shift selesai
...
begin
  ...
  if p_action = 'IN' then
    ...
    -- FIX BARU: Tolak masuk jika shift sudah berakhir
    if v_current_time > sched.end_time then
      return json_build_object('success', false, 'error', 'SHIFT_MISSED', 'message', 'Anda melewatkan jam absen untuk shift ini');
    end if;

export default function TaskList({ tasks, formatDate, openTaskDetail, openEditTaskModal, handleDeleteTask, currentUserRole }) {
  return (
    <div className="task-section" style={{ flex: '2', minWidth: '300px', marginTop: '2rem' }}>
      ...
      <div className="event-grid">
        {tasks.map(task => {
          const canEditDelete = currentUserRole === 'dosen' || task.creator?.role === 'panitia';
          ...
          return (
            <div key={task.id} className="task-card">
              ...
              <div className="task-actions" style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
                <button type="button" onClick={() => openTaskDetail(task, currentUserRole === 'dosen' ? 'detail_dosen' : 'detail_panitia')} className="btn btn-primary btn-small">Buka Task</button>
                {openEditTaskModal && canEditDelete && (
                ...
                {handleDeleteTask && canEditDelete && (
